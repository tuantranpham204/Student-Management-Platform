...
MAJOR_COLUMNS = ('id', 'name', 'department_id')

def _map_row_to_major(row: tuple) -> Optional[sn]:
    if not row:
        return None
    return sn(**dict(zip(MAJOR_COLUMNS, row)))

def get_all_majors() -> List[sn]:
    query = "SELECT id, name, department_id FROM majors"
    cursor.execute(query)
    rows = cursor.fetchall()
    return [_map_row_to_major(row) for row in rows]

def get_major_by_id(id: int) -> Optional[sn]:
    query = "SELECT id, name, department_id FROM majors WHERE id = %s"
    cursor.execute(query, (id,))
    row = cursor.fetchone()
    return _map_row_to_major(row)
