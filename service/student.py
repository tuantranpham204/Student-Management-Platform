...
def add_student(student: sn) -> bool:
    query = """
            INSERT INTO students (sid, fname, lname, dob, address, cid, phone, email, \
...
def select_binds(student: dict):
    ...
    if student.get("fname"):
        if params != 0: where += ' AND'
        where += f" s.fname LIKE '%{student['fname']}%' "
        params += 1

    if student.get("lname"):
        if params != 0: where += ' AND'
        where += f" s.lname LIKE '%{student['lname']}%' "
        params += 1

    if student.get("dob") is not None:
        if params != 0: where += ' AND'
        where += ' s.dob = %s '
    ...
    if student.get("gender") is not None:
        if params != 0: where += ' AND'
        where += ' s.gender = %s '
    ...
    if student.get("departmental_class_id"):
        if params != 0: where += ' AND'
...
def update_binds(student: dict):
    ...
    if student.get("img"):
        ...
        binds.append(student["img"])
    if student.get("departmental_class_id"):
        set += f" s.departmental_class_id = %s ,"
        binds.append(student["departmental_class_id"])
